...
# # Run 1 — concatenated
# python run_dpo.py --output_dir=./out_concat ...
# # (trong main script, set training_args.nnm_target = "concatenated")

# # Run 2 — chosen only
# python run_dpo.py --output_dir=./out_chosen ...
# # (set training_args.nnm_target = "chosen")

# # Run 3 — both with separate weights
# python run_dpo.py --output_dir=./out_both ...
# # (set training_args.nnm_target = "both"
# #  set training_args.nnm_rejected_weight = 0.3)
import math
import inspect
import random
import warnings
...
# ── Core TRL utils (stable across versions) ──────────────────────────────────
...
# RunningMoments — removed in some TRL versions
try:
...
# cap_exp — removed in some TRL versions
try:
    from trl.trainer.utils import cap_exp
except ImportError:
    def cap_exp(x, cap=1e9):
        return torch.clamp(x.exp(), max=cap)

# peft_module_casting_to_bf16 — moved in some TRL versions
try:
    from trl.trainer.utils import peft_module_casting_to_bf16
except ImportError:
    try:
        from trl.import_utils import peft_module_casting_to_bf16
    except ImportError:
...
# SyncRefModelCallback — added in TRL ~0.8, may be missing in older/newer
try:
...
# trl_sanitze_kwargs_for_tagging — typo was fixed to "sanitize" in newer TRL
try:
    from trl.trainer.utils import trl_sanitze_kwargs_for_tagging          # old (typo)
except ImportError:
    try:
        from trl.trainer.utils import trl_sanitize_kwargs_for_tagging as trl_sanitze_kwargs_for_tagging  # new (fixed)
    except ImportError:
...
# SAU:
        if getattr(args, "max_target_length", None) is None and self.is_encoder_decoder:
            warnings.warn(
            ...
            if not hasattr(args, "max_target_length"):
                object.__setattr__(args, "max_target_length", 128)
            else:
                args.max_target_length = 128
        self.max_target_length = getattr(args, "max_target_length", 128)
        ...
        with torch.no_grad(), compte_ref_context_manager():
            if self.ref_model is None:
                with self.null_ref_context():
                    out = self.concatenated_forward(self.model, self.ref_model, padded_batch)
            else:
                out = self.concatenated_forward(self.ref_model, self.ref_model, padded_batch)

        # First two outputs are chosen/rejected logps regardless of NNM
        reference_chosen_logps = out[0]
        reference_rejected_logps = out[1]
...
# ═══ NNM loss ═══
    ...
    def get_batch_loss_metrics(
        ...
        # Wrap back to keep return type consistent
        losses = losses.unsqueeze(0) if losses.dim() == 0 else losses
    ...
    def evaluation_loop(
        ...
        if self.generate_during_eval:
            ...
            policy_output_decoded, ref_output_decoded = self.generate_batch_samples(self.model, random_batch)
