...
python eval/merge_model.py \
  ...
  --adapter outputs/qwen2.5-1.5B-it-sft \
  --output ${CKPT_DIR}/qwen2.5-1.5B-it-sft

python eval/merge_model.py \
  --base_model Qwen/Qwen2.5-1.5B-Instruct \
  --adapter outputs/qwen2.5-1.5B-it-distillm2 \
  --output ${CKPT_DIR}/qwen2.5-1.5B-it-distillm2

python eval/merge_model.py \
  --base_model Qwen/Qwen2.5-1.5B-Instruct \
  --adapter outputs/qwen2.5-1.5B-it-distillm2-1epoch \
  --output ${CKPT_DIR}/qwen2.5-1.5B-it-distillm2-1epoch

run_eval \
    "qwen2.5-1.5B-it-sft-checkpoint-1149" \
    "pretrained=${CKPT_DIR}/qwen2.5-1.5B-it-sft,tensor_parallel_size=${TP},dtype=float16,gpu_memory_utilization=0.85,trust_remote_code=True"

run_eval \
    "qwen2.5-1.5B-it-distillm2-checkpoint-1869" \
    "pretrained=${CKPT_DIR}/qwen2.5-1.5B-it-distillm2,tensor_parallel_size=${TP},dtype=float16,gpu_memory_utilization=0.85,trust_remote_code=True"

run_eval \
    "qwen2.5-1.5B-it-distillm2-1epoch-checkpoint-2492" \
    "pretrained=${CKPT_DIR}/qwen2.5-1.5B-it-distillm2-1epoch,tensor_parallel_size=${TP},dtype=float16,gpu_memory_utilization=0.85,trust_remote_code=True"
